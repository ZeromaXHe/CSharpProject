...
namespace ConsoleApplicationLeetCodeSolutions.lcp
{
    ...
    {
        /// <summary>
        /// 执行用时：556 ms, 在所有 C# 提交中击败了 50.00% 的用户
        /// 内存消耗：73.7 MB, 在所有 C# 提交中击败了 100.00% 的用户
        /// 通过测试用例：105 / 105
        /// </summary>
        ...
        public int[] GetTriggerTime(int[][] increase, int[][] requirements)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                var sorted = requirements.Select(r => r[i])
                    .Zip(count, (r, c) => (r, c))
                    .OrderBy(tuple => tuple.r)
                    .ToArray();
                int sum = 0;
                int index = 0;
                while (index < n && sorted[index].r == 0) result[i, sorted[index++].c] = 0;
                int j = 0;
                while (j < increase.Length && index < n)
                {
                    sum += increase[j][i];
                    while (index < n && sum >= sorted[index].r) result[i, sorted[index++].c] = j + 1;
                    j++;
                }
            }
            ...
            for (int i = 0; i < n; i++)
            {
                int max = Math.Max(Math.Max(result[0, i], result[1, i]), result[2, i]);
                res[i] = max == Int32.MaxValue ? -1 : max;
            }

...
namespace Ch05Ex05
{
    ...
    internal class Program
    {
        public static void Main(string[] args)
        {
            ...
            foreach (var friendName in friendNames)
            {
                switch (friendName)
                {
                    case string t when t.StartsWith("T"):
                        WriteLine(
                            $"This friend's name starts with a 'T': {friendName} and is {t.Length - 1} letters long.");
                        break;
            ...
            WriteLine($"{total} total integers, {counter} integers with a value other than 0 or null" +
